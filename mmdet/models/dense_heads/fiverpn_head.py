...
class Improved3AttentionModule(nn.Module):
    ...
    def forward(self, x):
        ...
        x_fft_shifted = torch.fft.fftshift(x_fft)  # 将频谱移到中心
        x_fft_processed = torch.fft.ifft2(x_fft_shifted)  # 应用逆傅里叶变换
        x_fft_real = x_fft_processed.real  # 取实部
        S0 = self.fft_conv(x_fft_real)  # 应用3x3卷积
        S0 = self.bn_fft(S0)
        S0 = self.relu_fft(S0)
        ...
        return attn * u  # 将结果与原始输入u相乘，输出最终结果
