...
class FFDNConvFCBBoxHead(BBoxHead):
    ...
    def _add_conv_fc_branch(self,
        ...
        if num_branch_fcs > 0:
            rated branches, also consider self.num_shared_fcs
            if (is_shared
...
@MODELS.register_module()
class FFDNShared2FCBBoxHead(FFDNConvFCBBoxHead):
...
@MODELS.register_module()
class FFDNShared4Conv1FCBBoxHead(FFDNConvFCBBoxHead):
...
class FFDN(nn.Module):  
    def __init__(self, down_dim):
        ...
        self.fuse = nn.Sequential(
            nn.Conv2d(4 * down_dim, 256, kernel_size=1),  
            nn.BatchNorm2d(256),
            nn.PReLU()
        )
