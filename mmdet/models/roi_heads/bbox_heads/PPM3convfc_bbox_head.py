...
class PPM3ConvFCBBoxHead(BBoxHead):
    ...
    def __init__(self,
        ...
        # self.s2att=CombinedAttention(channel=256)##########
...
@MODELS.register_module()
class PPM3Shared2FCBBoxHead(PPM3ConvFCBBoxHead):
...
@MODELS.register_module()
class PPM3Shared4Conv1FCBBoxHead(PPM3ConvFCBBoxHead):
...
class PPM(nn.Module):  # PSPNet中的金字塔池化模块
    def __init__(self, down_dim):
        ...
        self.down_conv = nn.Sequential(
            nn.Conv2d(256, down_dim, 3, padding=1),
        ...
        # 使用卷积进行上采样
        self.up_conv1 = nn.Conv2d(down_dim, down_dim, kernel_size=3, padding=1)
        self.up_conv2 = nn.Conv2d(down_dim, down_dim, kernel_size=3, padding=1)
        self.up_conv3 = nn.Conv2d(down_dim, down_dim, kernel_size=3, padding=1)
        self.up_conv4 = nn.Conv2d(down_dim, down_dim, kernel_size=3, padding=1)

        # 融合不同尺度的特征
        self.fuse = nn.Sequential(
            nn.Conv2d(4 * down_dim, 256, kernel_size=1),  # 输出通道数设为256
            nn.BatchNorm2d(256),
            nn.PReLU()
        )
