...
def parse_args():
    ...
    parser.add_argument('--score-thr', type=float, default=0.3, help='score threshold to filter masks')
...
def calculate_confusion_matrix(dataset, results, score_thr=0, nms_iou_thr=None, tp_iou_thr=0.5):
    ...
    for idx, per_img_res in enumerate(results):
        res_masks = per_img_res['pred_instances']  # 处理分割结果
        gts = dataset.get_data_info(idx)['instances']
        analyze_per_img_dets(confusion_matrix, gts, res_masks, score_thr, tp_iou_thr, nms_iou_thr)
        prog_bar.update()
...
def analyze_per_img_dets(confusion_matrix, gts, result, score_thr=0, tp_iou_thr=0.5, nms_iou_thr=None):
    ...
    if isinstance(result['masks'], list):
        print("Result masks is a list, length:", len(result['masks']))
    else:
        print("Result masks shape:", result['masks'].shape)

    unique_label = np.unique(result['labels'])

    for det_label in unique_label:
        # 生成布尔索引 mask
        mask = (result['labels'] == det_label)  # 确保 mask 是布尔张量
        mask_indices = np.where(mask)[0]  # 获取索引的整数位置
        print(f"Processing label: {det_label}, Mask indices: {mask_indices}")

        # 处理分割掩码
        masks = [result['masks'][i] for i in mask_indices]  # 获取匹配的掩码
        det_bboxes = []
        det_scores = result['scores'][mask_indices]  # 提取分数

        # 从分割掩码提取外接矩形
        for mask in masks:
            mask = np.array(mask)  # 确保掩码为 NumPy 数组
            contours, _ = cv2.findContours(mask.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            for contour in contours:
                x, y, w, h = cv2.boundingRect(contour)
                det_bboxes.append([x, y, x + w, y + h])

        if not det_bboxes:  # 如果没有生成 bboxes，跳过
            continue
